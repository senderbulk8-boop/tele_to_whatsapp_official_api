...
# Sender WhatsApp Business number: 8104894648 (PHONE_NUMBER_ID secret)
# Receiver (jahan message jana hai): 7737781986
def _india_number(raw):
    digits = re.sub(r"\D", "", raw or "")
    if len(digits) == 10:
        return "91" + digits
    return digits or "917737781986"


RECEIVER = _india_number(os.getenv("WHATSAPP_TO") or "917737781986")

# Optional: 24h window band ho to isi approved template se bhejo (body me {{1}})
TEMPLATE_NAME = (os.getenv("WHATSAPP_TEMPLATE_NAME") or "").strip()
TEMPLATE_LANG = os.getenv("WHATSAPP_TEMPLATE_LANG") or "en"

LAST_OFFSET = 826690506  # Code khud update karega
# ====================================================
...
def explain_wa_error(code, body):
    ...
    if code == 131047:
        print(
            "   24-hour window band hai. Free text tabhi jayegi jab "
            "7737781986 wala phone WhatsApp pe 8104894648 ko message kare (Hi).\n"
            "   Uske baad 24 ghante session messages chalenge.\n"
            "   Permanent fix: Meta me template banao (body {{1}}) aur secret "
            "WHATSAPP_TEMPLATE_NAME set karo."
        )
    elif code == 131030:
        print(
            "   Test/dev mode allow-list. Meta Developer Dashboard → WhatsApp → "
            "API Setup → To number me 917737781986 add karo aur OTP verify karo."
        )
    elif code == 131026:
        ...
        print("   Sender aur receiver same number hai. Receiver 917737781986 hona chahiye, sender 8104894648.")
...
def send_media(file_bytes, mime, caption="", filename="file.bin"):
    ...
    if not media_id:
        # upload fail ho to caption/text fallback
        if caption:
...
def forward_message(message):
    user = message.get("from", {}).get("first_name") or message.get("sender_chat", {}).get("title") or "Channel"
    ...
    # Unknown type — text fallback so message lose na ho
    kind = ", ".join(k for k in message.keys() if k not in ("from", "chat", "date", "message_id"))
...
def main():
    ...
    print(f"📌 Receiver: {RECEIVER}  (7737781986)")
    print("📌 Groups/channels on WhatsApp: Official Meta API se skip (possible nahi / alag Groups API)")
    ...
    try:
        ...
        for update in updates:
            ...
            if not ok:
                print(f"❌ Forward failed for Telegram update_id={update_id} — offset yahin rukega, next run retry karega")
                failed = True
                break
